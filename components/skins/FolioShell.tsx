  ...
  return (
    <div className="m2-app folio-mobile" data-mode={room === "studio" ? "studio" : "perform"} data-room={room} data-skin={skin}>
      ...
      <main className="folio-mobile-main">
        ...
        {room === "play" ? marketView === "chart"
          ? <FolioHomeMobile surface={surface} />
          : <MobilePerform props={surface} channels={channels} sent={surface.sentinel} livePnl={surface.livePnl} marketView={marketView} onMarketViewChange={setMarketView} />
          : room === "studio" ? <FolioChannelsMobile surface={surface} openSlug={openSlug} setOpenSlug={setOpenSlug} onAddChannel={() => setAddOpen(true)} onOpenSettings={() => setSettingsOpen(true)} />
            : room === "book" ? <FolioBookMobile surface={surface} onViewMarket={openMarket} />
              : room === "review" ? <FolioReviewMobile surface={surface} channels={channels} />
                : room === "ops" ? <FolioOpsMobile surface={surface} channels={channels} onOpenSettings={() => setSettingsOpen(true)} />
                  : <MobileDeskRoom room={room} props={surface} channels={channels} livePnl={surface.livePnl} onViewMarket={openMarket} onOpenSettings={() => setSettingsOpen(true)} />}
      </main>

      <nav className="folio-mobile-nav" aria-label="Folio mobile workspaces">
        {MOBILE_NAV.map((item) => <button type="button" key={item.id} className={room === item.id ? "on" : ""} onClick={() => { if (item.id === "play") setMarketView("chart"); setRoom(item.id); }} aria-label={item.label}><i>{item.mark}</i><span>{item.label}</span></button>)}
      </nav>
