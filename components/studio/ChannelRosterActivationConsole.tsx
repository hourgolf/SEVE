  ...
  return <section className="mix-bank roster-activation-console" aria-label="Atomic channel roster activation console">
    ...
    <div className="roster-console-body">
      ...
      {roster.previewSession && <div className={`roster-preview ${roster.previewSession.preview.state}`}>
        ...
        {!!roster.previewSession.preview.capacity?.metrics.length && <details><summary>PORTFOLIO CAPACITY RECEIPT</summary>{roster.previewSession.preview.capacity.staticDollarEnvelope === "not-proven-for-full-roster" && <p>Broker affordability is required at each MACD entry. A fixed dollar ceiling for the full roster is not proven; the reference percentage limits below do not cap MACD.</p>}{roster.previewSession.preview.capacity.metrics.map((metric) => <span key={metric.id} className={metric.state}><code>{metric.id}</code><b>{metric.projected == null ? "Quote-dependent / reference " : `${metric.projected} / `}{metric.limit}</b></span>)}</details>}
