...
export function CanaryCommandCenter({
  ...
  return <section className={`canary-command-center${compact ? " compact" : ""}`} aria-label="Canary command center">
    <header>
      <span><small>NEXT SESSION · PAPER CANARY</small><b>CANARY COMMAND CENTER</b></span>
      <em className={receiptBound ? "ready" : "blocked"}>{receiptBound ? "SEALED · PREOPEN GATE NEXT" : "AUTHORITY BLOCKED"}</em>
    </header>
    <div className="canary-command-body">
