...
export function ReviewSessionScorecard({ evidence }: { evidence: ReturnType<typeof useDailyReports> }) {
  ...
  return <section className="rvw-session-scorecard" aria-label="Last completed trading session">
    ...
    <div className="rvw-session-metrics">
      ...
      <span><small>SAMPLED HELD PEAK</small><b>{model.averageBestMovePct == null ? "—" : `+${model.averageBestMovePct.toFixed(1)}%`}</b><em>average best move while open</em></span>
      <span><small>HELD-MARK RATIO</small><b className={model.retainedPct != null && model.retainedPct >= 50 ? "pos" : "neg"}>{model.retainedPct == null ? "—" : `${model.retainedPct.toFixed(1)}%`}</b><em>{model.retainedPct == null ? (report.digest.peakDiagnostic?.signedRatio != null && report.digest.peakDiagnostic.signedRatio < 0 ? "coherent subset finished below entry" : "no comparable peak evidence") : "coherent subset only; executable capture unknown"}</em></span>
    </div>
