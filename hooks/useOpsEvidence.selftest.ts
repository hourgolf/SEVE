...
assert.match(hook, /pollMs = 120_000, deepEnabled = true/, "OPS evidence should default to a slow cadence");
assert.match(hook, /settle\("broker"[\s\S]*if \(!deepEnabled\) return;/, "non-OPS workspaces should read only compact broker truth");
...
assert.match(hook, /expireInitialLoads\(previous, Date\.now\(\), deepEnabled\)/, "an unresolved browser session must become an explicit read error");
