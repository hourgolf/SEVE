...
const pnlReader = read("./windowedEvidenceRead.ts");
...
assert.match(pnlHook, /readWindowedPositions\(sb/);
assert.match(pnlHook, /readWindowedExecutionRoutes\(sb/);
assert.match(pnlReader, /from\("execution_observations"\)/);
assert.match(pnlReader, /select\("id,position_id,account_id,event_at", \{ count: "exact" \}\)/);
