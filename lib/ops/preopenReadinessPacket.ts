import type { OperationalReleaseContract } from "./preopenReadinessEngine";

export interface PreopenReadinessPacketInput {
  sessionDate: string;
  contract: OperationalReleaseContract;
  envFile: string;
}

export function renderPreopenReadinessPacket(input: PreopenReadinessPacketInput): string {
  const { sessionDate, contract, envFile } = input;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(sessionDate)) {
    throw new Error("sessionDate must be YYYY-MM-DD");
  }
  const accountCount = new Set(contract.requiredAccountIds).size;
  return `# SEVE pre-open readiness packet — ${sessionDate}
...
- Release: \`${contract.releaseId}\`
- Configuration SHA-256: \`${contract.configurationSha256}\`
- Strategy worker: \`${contract.strategyWorkerVersion}\`
- Runtime: \`${contract.runtimeVersion}\`
- Sealed roots: ${contract.roots.length}
- Release-required accounts: ${accountCount}
- Broker boundary: \`${contract.paperOrigin}\`
- Feeds: stock \`${contract.stockFeed}\` · options \`${contract.optionFeed}\`
...
\`\`\`bash
npm run preopen -- --env-file ${envFile}
\`\`\`
