...
export const DECISION_ATLAS_IWM_PROMOTION_VERSION =
  "decision-atlas-iwm-promotion-v1" as const;

export const DECISION_ATLAS_IWM_PROMOTION = Object.freeze({
  slug: "vb-ribbon-cross-iwm" as const,
  channelId: "7a4f2a8e-232b-49b1-83cf-5e0d7a26c502",
  displayName: "VB · ribbon cross (IWM)",
  hypothesis:
    "Trade only the first 9/21 moving-average ribbon cross backed by at least 1.2 relative volume before 14:00 ET.",
  familyId: "IWM-RIBBON-CROSS",
  accountId: "56daa293-e6bc-447d-83ac-2bfafb4d0ac1",
  accountName: "LAB" as const,
  accountRole: "LAB" as const,
  collisionDomain: "rc54-lab" as const,
  quantity: 2 as const,
  premiumCap: 3,
  maxDebitUsd: 600,
  maxRiskUsd: 180,
  takeProfitPct: 25,
  premiumStopPct: 30,
  maxEntriesPerSession: 1 as const,
  priority: 1,
  evidenceRef:
    "decision-atlas:vb-ribbon-cross-iwm:first-entry:27-sessions:through-2026-08-14",
});
...
function cartridge(input: {
  ...
  const candidate = DECISION_ATLAS_IWM_PROMOTION;
  return {
    schemaVersion: 1,
    identity: {
      slug: candidate.slug,
    ...
    risk: {
      ...
      concentrationTags: ["IWM", "US-INDEX-LONG-PREMIUM"],
    },
    management: {
...
export function buildDecisionAtlasIwmRegistration(input: {
  sourceContentHash: string;
  runtimeVersion: string;
  runtimeSourceCommit: string;
  registeredAt: string;
  ...
  const identity = contentHash({
    version: DECISION_ATLAS_IWM_PROMOTION_VERSION,
    candidate: DECISION_ATLAS_IWM_PROMOTION.slug,
  ...
  return registerResearchChannel({
    id: `research:${DECISION_ATLAS_IWM_PROMOTION.slug}:qualified-${identity}`,
    channelId: DECISION_ATLAS_IWM_PROMOTION.channelId,
    slug: DECISION_ATLAS_IWM_PROMOTION.slug,
