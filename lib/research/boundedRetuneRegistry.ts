...
export interface BoundedRetuneExperimentDefinition {
  ...
  channel: string;
  strategistId: string;
  cohortStartSession: typeof PRIORITY_A_RETUNE_COHORT_START;
...
const define = (input: DefinitionInput): BoundedRetuneExperimentDefinition => Object.freeze({
  schema: BOUNDED_RETUNE_REGISTRY_SCHEMA,
  experimentId: `priority-a:${input.channel}:${input.variable}:v1`,
  priority: "A",
  cohortStartSession: PRIORITY_A_RETUNE_COHORT_START,
...
const entryCap = (input: {
  ...
  cap: 1 | 2; sessions: number; outcomes: number;
  ...
  sourceEvidence: { throughSession: "2026-08-07", sessions: input.sessions,
...
const BY_ID = new Map(PRIORITY_A_BOUNDED_RETUNES.map((row) => [row.experimentId, row]));
