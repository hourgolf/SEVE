...
function loadLog(): Emit {
  ...
  for (const p of ["days", "from", "to", "underlying", "spec", "trail", "trail-until", "breakeven", "breakeven-lock", "late-cutoff", "late-max", "fill-cross"] as const) { const v = argStr(p, ""); if (v) args.push(`--${p}`, v); }
