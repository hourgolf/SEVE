...
create table public.vb_candidate_receipts (
  ...
  channel_version          text not null check (length(channel_version) between 1 and 160),
  ...
  blocked_reason           text not null check (blocked_reason in ('not_armed', 'halted', 'cost_gate', 'stale_chain')),
...
create table public.vb_exact_path_receipts (
  ...
  check (last_quote_at >= first_quote_at),
