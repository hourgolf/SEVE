-- Server-only draft proposal creation.
--
-- This migration adds one atomic write seam. It does not grant clients write
-- access, validate or approve a proposal, authorize activation, create an
-- activation receipt, promote a spec/manifest, or alter any runtime reader.

begin;
set local lock_timeout = '5s';
set local statement_timeout = '30s';

create or replace function public.create_channel_change_proposal_draft(
  p_proposal_id uuid,
...
begin
  -- Serialize retries for one idempotency key without locking unrelated drafts.
  ...
  if p_author_id is null
      or p_author_id !~ '^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$' then
    raise exception 'operator identity must be a canonical UUID';
  end if;
  if p_author_id::uuid::text <> p_author_id then
    raise exception 'operator identity must be a canonical UUID';
  end if;
  if p_reason is null or length(btrim(p_reason)) < 8 or length(btrim(p_reason)) > 2000 then
    raise exception 'proposal reason must contain 8 to 2000 characters';
  end if;
  if p_change_class <> 'bounded-parameter' then
    raise exception 'this write slice accepts bounded-parameter proposals only';
  end if;
  if jsonb_typeof(p_proposed_patch) <> 'object'
      or p_proposed_patch = '{}'::jsonb
      or p_proposed_patch ?| array[
        'schemaVersion', 'id', 'channelId', 'slug', 'createdAt', 'createdBy',
        'parentVersionId', 'status', 'validFrom', 'validUntil', 'contentHash'
      ] then
    raise exception 'proposal patch is empty or contains immutable fields';
  end if;
  if exists (
    select 1
    from jsonb_object_keys(p_proposed_patch) field
    where field not in ('quantity', 'maxDebitUsd', 'takeProfit', 'stopLoss', 'riskLimits')
  ) then
    raise exception 'proposal patch contains a field outside the bounded write slice';
  end if;
  ...
  if base_row.id is null then
    raise exception using errcode = 'P0002', message = 'proposal base specification is missing';
  end if;
  if base_row.content_hash <> p_base_content_hash then
    raise exception using errcode = '40001', message = 'proposal base hash drifted';
  end if;
  ...
  if p_proposed_spec ->> 'strategyIdentity' is distinct from base_row.strategy_identity
      ...
      or p_proposed_spec ->> 'collisionDomain' is distinct from base_row.collision_domain then
    raise exception 'bounded proposal attempted a governed or code-level spec change';
  end if;

  if p_proposed_spec -> 'quantity'
        is distinct from coalesce(p_proposed_patch -> 'quantity', to_jsonb(base_row.quantity))
      or p_proposed_spec -> 'maxDebitUsd'
        is distinct from coalesce(p_proposed_patch -> 'maxDebitUsd', to_jsonb(base_row.max_debit_usd))
      or p_proposed_spec -> 'takeProfit'
        is distinct from coalesce(p_proposed_patch -> 'takeProfit', base_row.take_profit)
      or p_proposed_spec -> 'stopLoss'
        is distinct from coalesce(p_proposed_patch -> 'stopLoss', base_row.stop_loss)
      or p_proposed_spec -> 'riskLimits'
        is distinct from coalesce(p_proposed_patch -> 'riskLimits', base_row.risk_limits) then
    raise exception 'proposed specification does not match its bounded patch';
  end if;
...
revoke all on function public.create_channel_change_proposal_draft(
  uuid, text, text, text, jsonb, jsonb, text, jsonb, text, text,
  jsonb, jsonb, jsonb, timestamptz
) from public, anon, authenticated;
grant execute on function public.create_channel_change_proposal_draft(
  uuid, text, text, text, jsonb, jsonb, text, jsonb, text, text,
  jsonb, jsonb, jsonb, timestamptz
) to service_role;

comment on function public.create_channel_change_proposal_draft(
  uuid, text, text, text, jsonb, jsonb, text, jsonb, text, text,
  jsonb, jsonb, jsonb, timestamptz
) is
  'Service-role-only, atomic and idempotent creation of a draft child spec and draft proposal. Never activation authority.';
