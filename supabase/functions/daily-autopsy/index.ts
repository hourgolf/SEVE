...
const NARRATE_SYSTEM = `You are SEVE's daily paper-trading evidence analyst. You receive one day's deterministic digest and historical findings. Apply the evidence rules below to the supplied records; prior conclusions are context to verify.
For each channel, explain its documented intent, observed entry location/conviction, outcome and uncertainty. Separate strategy hypotheses from verified execution/reporting defects. A reconciled close has ambiguous cause without exact broker/action evidence; do not infer a manual close, collision or expiry. Identify recurrence only when compatible prior evidence supports it. Retain every supplied diagnostic flag, while distinguishing the flag from a proven failure and its cause. A falsifiable next test is preferable to a guessed fix.
Return concise marketSummary, channel assessments, systemFindings and topActions using the existing schema. Top actions may be empty when no action is supported. Prioritize genuine correctness and evidence gaps over tuning. Do not manufacture three to five actions or a system bug from a quota.
...
// Preserve each diagnostic when narration is absent without inventing its cause
// or turning an observed pattern into an instruction to change trading controls.
const FLAW_META: Record<string, { category: string; severity: string; hypothesis: string; experiment: string }> = {
  insta_exit:       { category: "system",   severity: "high", hypothesis: "Cause unverified: short holds may reflect the intended scalp or native stop, an execution problem, or a reconciled close. Hold duration alone does not identify the exit owner.", experiment: "Join exact broker exits and root receipts to the entry-bound manager, then replay its native triggers on continuous executable quotes. Retain missing-trigger cases as unknown." },
  exit_monoculture: { category: "system",   severity: "med",  hypothesis: "Cause unverified: a dominant exit reason can match an intentional all-out manager or a repeated market outcome. It does not by itself establish missing management.", experiment: "Compare recorded exit reasons with each original manager's permitted triggers and exact order lineage. Test any exit alternative on paired opportunities with native stops preserved before proposing a change." },
  size_pinned:      { category: "config",   severity: "low",  hypothesis: "Cause unverified: maximum-size entries can reflect an intentional fixed quantity, a later release override, or affordability under the sizing inputs. This flag does not establish which policy applied.", experiment: "Compare receipt-bound sizing policy with provisional, release-requested, submitted and filled quantities and nominal-stop exposure. Mark missing stages unknown; establish the intended contract before proposing quantity changes." },
  gate_starved:     { category: "system",   severity: "med",  hypothesis: "Cause unverified: a low acted-to-recorded-signal ratio can include repeated state evaluations, observing channels and intentional admission controls. Recorded signals are not a complete independent raw-entry universe.", experiment: "Group distinct setup episodes where identifiable and inspect each actually visited gate, policy era and current posture. Separate unqueried quotes and unevaluated stages from failed entry predicates before assessing a gate." },
  negative_day:     { category: "strategy", severity: "low",  hypothesis: "A net-negative session is observed; it does not establish off-regime strategy failure or justify a roster change.", experiment: "Compare configuration-compatible sessions against the channel's actual underlying, separating entry selection from native exit conversion. Report tails, concentration and missing evidence before a roster proposal." },
};
