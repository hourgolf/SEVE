...
const osi = (expISO: string, strike: number, type: "call" | "put") => { const [y, m, d] = expISO.split("-"); return "SPY   " + y.slice(2) + m + d + (type === "call" ? "C" : "P") + String(Math.round(strike * 1000)).padStart(8, "0"); };
...
async function dayRanges(sb: ReturnType<typeof createClient>): Promise<DayRange[]> {
  ...
  for (let from = 0; ; from += 1000) {
    const { data, error } = await sb.from("underlying_bars").select("ts,low,high").eq("symbol", "SPY").order("ts", { ascending: true }).range(from, from + 999);
...
async function fetchDay(symbols: string[], day: string): Promise<Record<string, unknown>[]> {
  ...
  for (const line of t.trim().split("\n")) {
    ...
    const occ = "SPY" + yymmdd + cp + sym.slice(13);
