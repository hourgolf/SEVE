...
function overlayRoot(channel: ChannelConfig, root: Readonly<ReceiptBoundRuntimeRoot>): ChannelConfig {
  ...
  return {
    ...
    daily_stop_usd: root.dailyEntryLossLatchUsd ?? 0,
    daily_target_usd: root.dailyEntryProfitLatchUsd ?? 0,
    underlying_stop_pct: root.underlyingStopPct ?? 0,
    muted: false,
    soloed: false,
    boosted: false,
    event_policy: root.eventPolicy ?? "standdown",
    ...
    pyramid_adds: root.pyramidAdds ?? 0,
    stall_minutes: root.stallMinutes ?? 0,
    stall_max_favor_pct: root.stallMaxFavorablePct ?? 0,
    gap_min: root.gapMinPct ?? 0,
