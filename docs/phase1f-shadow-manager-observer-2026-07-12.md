...
The causal policy was extracted from the retrospective research lab into
`engine/managerPolicy.ts`; the research lab and forward observer therefore use
the same eight registered definitions without shipping the rest of the Databento
research stack in this runtime branch:
...
2. The research-branch `manager-lab-selftest` passes (shared-policy parity).
3. `manager-shadow-selftest` passes (causality, restart identity, cohort gate).
4. Worker and root TypeScript checks pass.
5. Merge this scoped branch to `main`, then manually deploy Railway service
